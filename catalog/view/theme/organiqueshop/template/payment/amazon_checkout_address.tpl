  ...
  $(document).ready(function(){
    $('#continue-button').click(function(){
      ...
      if ($('input[name="addressSelected"]').val() == '0') {
        $('#amazon-address-widget').before('<div class="warning"><span class="fa fa-exclamation-circle fa-lg margin-right10"></span><?php echo $error_shipping_address ?></div>');
      } else if($('input[name="shipping_method"]:checked').length == 0) {
        $('#amazon-address-widget').before('<div class="warning"><span class="fa fa-exclamation-circle fa-lg margin-right10"></span><?php echo $error_shipping ?></div>');
      } else {
        $.ajax({
    ...
    new CBA.Widgets.AddressWidget({
      ...
      onAddressSelect: function(widget) {
        ...
        $.get('<?php echo $shipping_quotes ?>', {}, function(data) {
          ...
          if (data.error) {
            $('#amazon-address-widget').before('<div class="warning"><span class="fa fa-exclamation-circle fa-lg margin-right10"></span>' + data.error + '</div>');
          } elseif (data.quotes) {
            var html = '';

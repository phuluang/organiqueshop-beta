<?php
################################################################################
#  ชุดภาษาไทยหลังร้าน สำหรับ Opencart 1.4.6 โดย Somsak2004 วันที่ 3 เมษายน 2553
#  ปรับปรุง สำหรับรุ่น 1.4.9 โดย DomainHost360.com วันที่ 06/09/2553
################################################################################
# ผู้ให้การสนับสนุนโปรเจค Siam Opencart
#
# DomainHost360.com: บริการ Hosting สำหรับ Opencart พื้นที่ 10 GB ปีละ 999 บาท Server เร็วสุดในไทย
# Unitedsme.com : ผู้ให้บริการเช่าพื้นที่เว็บไซต์ จดโดเมน ระบบ Linux โทร. 081-454-9454 [คุณภูมิ]
# Net-LifeStyle.com : ผู้ให้บริการเช่าพื้นที่เว็บไซต์ จดโดเมน ระบบ Linux & Windows โทร. 086-338-5909 [คุณโสภณ]
# SiamWebThai.com : SEO ขั้นเทพ โปรโมทเว็บขั้นเซียน ออกแบบ พัฒนาเว็บไซต์ / ตามความต้องการ และถูกใจ Google 
# โทร. 086-549-9998 [คุณมานะ] ทำให้เว็บไซต์ของคุณติดหน้าแรกของ Google ตามคำที่คุณต้องการ!
################################################################################
# ขอแนะนำ Web hosting สำหรับ Open cart
#
#1. www.domainhost360.com
#2. www.hostzaa.com
#3. www.citec.us
#4. www.mehostdd.com
################################################################################

// Heading
$_['heading_title']    = 'ดาวน์โหลด';

// Text
$_['text_success']     = 'สำเร็จ: แก้ไขดาวน์โหลดเรียบร้อย!';

// Column
$_['column_name']      = 'ชื่อดาวน์โหลด';
$_['column_remaining'] = 'รวมดาวน์โหลดที่อนุญาติ';
$_['column_action']    = 'การกระทำ';

// Entry
$_['entry_name']       = 'ชื่อดาวน์โหลด:';
$_['entry_filename']   = 'ชื่อไฟล์:';
$_['entry_remaining']  = 'รวมดาวน์โหลดที่อนุญาติ:';
$_['entry_update']     = 'วางไฟล์สำหรับลูกค้าที่ซื้อไปก่อนหน้านี้:<br /><span class="help">เลือกตัวเลือกนี้เพื่อปรับปรุงไฟล์สำหรับลูกค้าที่สั่งซื้อไปก่อนหน้านี้.</span>';

// Error
$_['error_permission'] ='คำเตือน: คุณไม่ได้รับอนุญาตให้แก้ไขดาวน์โหลด!';
$_['error_name']       = 'ชื่อต้องอยุ่ระหว่าง 3 ถึง 255 ตัวอักษร!';
$_['error_filename']   = 'ชื่อไฟล์ต้องอยุ่ระหว่าง 3 ถึง 128 ตัวอักษร!';
$_['error_filetype']   = 'ชนิดไฟล์ไม่ถูกต้อง!';
$_['error_product']    = 'คำเตือน:ดาวน์โหลดนี้ไม่สามารถลบเพราะมีสินค้าอยู่ %s รายการ!';
?>
